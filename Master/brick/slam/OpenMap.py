...
class OpenMap:
    ...
    def drawPoint(self):
        ...
        if len(self.history) > 0:
            for node in self.history:
                self.mapimg = cv2.circle(self.mapimg, (node[1], node[0]), 1, (100, 0, 100), -1)  # 表示把圆填满
    ...
    def drawPose(self):
        ...
        if self.history_pose == True:
            self.history.append([self.y_p, self.x_p])  # 将历史位置转换成元组

    def display(self,x_m,y_m,theta,mapbytes,test=True):
    ...
    def display_mjpg(self,x_m,y_m,theta,mapbytes,test=True):
    ...
    def get_history_pose(self):  # 获取历史经过点
        self.history = []
        self.history_pose = True  # 用来判断是否该进行历史储存

    def del_history_pose(self):  # 清除历史经过点
        self.history = []
        self.history_pose = False  # 用来判断是否该进行历史储存
